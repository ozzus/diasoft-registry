...
project_name="${project_name:-diasoft-registry}"
...
COMMIT_AUTHOR_NAME="${GITLAB_USER_NAME:-${GITHUB_ACTOR:-diasoft-registry-ci}}"
...
yq -i '.deploymentMetadata.contractVersion = strenv(PROMOTION_SHA)' "$OVERLAY_FILE"
...
git commit -m "chore: promote registry ${TARGET_TEAM}-${TARGET_ENV} to ${promotion_sha}"
...
if [ "$PROMOTION_MODE" = "pr" ]; then
  ...
  if [ -n "${PLATFORM_INFRA_GITLAB_TOKEN:-}" ] && [ -n "${PLATFORM_INFRA_GITLAB_PROJECT_ID:-}" ]; then
    curl --fail-with-body --request POST \
      ...
      --data-urlencode "title=Promote diasoft-registry ${TARGET_TEAM}-${TARGET_ENV} to ${promotion_sha}" \
    ...
    curl --fail-with-body --request POST \
      ...
      --data "{\"title\":\"Promote diasoft-registry ${TARGET_TEAM}-${TARGET_ENV} to ${promotion_sha}\",\"head\":\"${BRANCH_NAME}\",\"base\":\"${PLATFORM_INFRA_BASE_BRANCH}\",\"body\":\"Automated promotion for ${project_name} at ${promotion_sha}.\"}" \
